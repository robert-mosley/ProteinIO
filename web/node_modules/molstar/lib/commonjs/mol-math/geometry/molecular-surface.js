...
exports.MolecularSurfaceCalculationParams = {
    ...
    resolution: param_definition_1.ParamDefinition.Numeric(0.5, { min: 0.01, max: 20, step: 0.01 }, { description: 'Grid resolution/cell spacing.' }),
    probePositions: param_definition_1.ParamDefinition.Numeric(36, { min: 12, max: 90, step: 1 }, { description: 'Number of positions tested for probe target intersection.' }),
};
...
async function calcMolecularSurface(ctx, position, boundary, maxRadius, box, props) {
    ...
    function obscured(x, y, z, a, b) {
        ...
        for (let j = 0, jl = neighbours.count; j < jl; ++j) {
            const ai = ordered_set_1.OrderedSet.getAt(indices, neighbours.indices[j]);
            if (ai !== a && ai !== b && singleAtomObscures(ai, x, y, z)) {
                lastClip = ai;
                return ai;
            }
    ...
    function projectPointsRange(begI, endI) {
        for (let i = begI; i < endI; ++i) {
            const j = ordered_set_1.OrderedSet.getAt(indices, i);
    ...
    const atob = (0, vec3_1.Vec3)();
    const mid = (0, vec3_1.Vec3)();
    const n1 = (0, vec3_1.Vec3)();
    const n2 = (0, vec3_1.Vec3)();
    /**
     * `a` and `b` must be resolved indices
     */
    function projectTorus(a, b) {
        ...
        const dmp = rA * cosA;
        vec3_1.Vec3.normalize(atob, atob);
        // Create normal to line
        normalToLine(n1, atob);
        vec3_1.Vec3.normalize(n1, n1);
        // Cross together for second normal vector
        vec3_1.Vec3.cross(n2, atob, n1);
        vec3_1.Vec3.normalize(n2, n2);
        // r is radius of circle of intersection
        const rInt = Math.sqrt(rA * rA - dmp * dmp);
        vec3_1.Vec3.scale(n1, n1, rInt);
        vec3_1.Vec3.scale(n2, n2, rInt);
        vec3_1.Vec3.scale(atob, atob, dmp);
        mid[0] = atob[0] + px[a];
        mid[1] = atob[1] + py[a];
        mid[2] = atob[2] + pz[a];
        lastClip = -1;
                    ...
                    for (let yi = begY; yi < endY; ++yi) {
                        ...
                        for (let zi = begZ; zi < endZ; ++zi) {
                            ...
                            if (current > 0.0 && dSq < (current * current)) {
                                ...
                                idData[idx] = id[ordered_set_1.OrderedSet.indexOf(indices, dp < 0.0 ? b : a)];
                            }
                        }
                    }
                }
    ...
    function projectToriiRange(begI, endI) {
        for (let i = begI; i < endI; ++i) {
            const k = ordered_set_1.OrderedSet.getAt(indices, i);
            lookup3d.find(px[k], py[k], pz[k], radius[k]);
            for (let j = 0, jl = neighbours.count; j < jl; ++j) {
                const l = ordered_set_1.OrderedSet.getAt(indices, neighbours.indices[j]);
                if (k < l)
                    projectTorus(k, l);
            }
        }
    ...
    const cellSize = vec3_1.Vec3.create(maxRadius, maxRadius, maxRadius);
    vec3_1.Vec3.scale(cellSize, cellSize, 2);
    const lookup3d = (0, grid_1.GridLookup3D)(position, boundary, cellSize);
    const neighbours = lookup3d.result;
    if (box === null)
        box = lookup3d.boundary.box;
    const { indices, x: px, y: py, z: pz, id, radius } = position;
    const n = ordered_set_1.OrderedSet.size(indices);
    const pad = maxRadius + resolution;
    const expandedBox = box3d_1.Box3D.expand((0, box3d_1.Box3D)(), box, vec3_1.Vec3.create(pad, pad, pad));
    const [minX, minY, minZ] = expandedBox.min;
    const scaledBox = box3d_1.Box3D.scale((0, box3d_1.Box3D)(), expandedBox, scaleFactor);
    const dim = box3d_1.Box3D.size((0, vec3_1.Vec3)(), scaledBox);
    vec3_1.Vec3.ceil(dim, dim);
    const [dimX, dimY, dimZ] = dim;
    const iu = dimZ, iv = dimY, iuv = iu * iv;
    const { cosTable, sinTable } = getAngleTables(probePositions);
    const space = tensor_1.Tensor.Space(dim, [0, 1, 2], Float32Array);
    const data = space.create();
    const idData = space.create();
    data.fill(-1001.0);
    idData.fill(-1);
    const gridx = (0, common_1.fillGridDim)(dimX, minX, resolution);
    const gridy = (0, common_1.fillGridDim)(dimY, minY, resolution);
    const gridz = (0, common_1.fillGridDim)(dimZ, minZ, resolution);
    ...
    const field = tensor_1.Tensor.create(space, data);
    const idField = tensor_1.Tensor.create(space, idData);
    const transform = mat4_1.Mat4.identity();
    mat4_1.Mat4.fromScaling(transform, vec3_1.Vec3.create(resolution, resolution, resolution));
