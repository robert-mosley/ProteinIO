...
const approx_1 = require("../../approx.js");
...
async function GaussianDensityCPU(ctx, position, box, radius, props) {
    ...
    let maxRadius = 0;
    for (let i = 0; i < n; ++i) {
        const r = radius(ordered_set_1.OrderedSet.getAt(indices, i)) + radiusOffset;
        if (maxRadius < r)
            maxRadius = r;
        radii[i] = r;
    }
    ...
    const gridx = (0, common_1.fillGridDim)(dim[0], min[0], resolution);
    const gridy = (0, common_1.fillGridDim)(dim[1], min[1], resolution);
    const gridz = (0, common_1.fillGridDim)(dim[2], min[2], resolution);
