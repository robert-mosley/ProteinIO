...
(function (TMAlign) {
    ...
    function calculateTMScore(xa, ya, alignA, alignB, transform, d0, normLength) {
        ...
        for (let i = 0; i < alignA.length; i++) {
            const ai = alignA[i];
            const bi = alignB[i];
            vec3_1.Vec3.set(vA, xa[ai][0], xa[ai][1], xa[ai][2]);
            vec3_1.Vec3.set(vB, ya[bi][0], ya[bi][1], ya[bi][2]);
            vec3_1.Vec3.transformMat4(vB, vB, transform);
            const distSq = vec3_1.Vec3.squaredDistance(vA, vB);
    ...
    function calculateRMSD(xa, ya, alignA, alignB, transform) {
        ...
        const vB = (0, vec3_1.Vec3)();
        for (let i = 0; i < alignA.length; i++) {
            vec3_1.Vec3.set(vA, xa[alignA[i]][0], xa[alignA[i]][1], xa[alignA[i]][2]);
            vec3_1.Vec3.set(vB, ya[alignB[i]][0], ya[alignB[i]][1], ya[alignB[i]][2]);
            vec3_1.Vec3.transformMat4(vB, vB, transform);
            sumSq += vec3_1.Vec3.squaredDistance(vA, vB);
        }
 ...
 */
class TMAlignState {
    constructor(xa, ya, lenA, lenB, d0A, d0Search, scoreD8) {
        this.bestScore = -1;
        this.bestAlignmentA = [];
        this.bestAlignmentB = [];
    ...
    optimizeFinalAlignment() {
        ...
        for (const cutoff of cutoffs) {
            ...
            for (let i = 0; i < this.bestAlignmentA.length; i++) {
                ...
                const v = vec3_1.Vec3.create(ya[bi][0], ya[bi][1], ya[bi][2]);
                vec3_1.Vec3.transformMat4(v, v, this.bestTransform);
            ...
            const score = this.scoreTM(this.bestAlignmentA, this.bestAlignmentB, transform, d0A, lenA);
            if (score > this.bestScore) {
                this.bestScore = score;
                this.bestTransform = mat4_1.Mat4.clone(transform);
            }
        }
        // Recompute final score
        this.bestScore = this.scoreTM(this.bestAlignmentA, this.bestAlignmentB, this.bestTransform, d0A, lenA);
    }
    /**
    ...
    scoreTM(alignA, alignB, transform, d0, normLen) {
        ...
        for (let i = 0; i < alignA.length; i++) {
            ...
            const bi = alignB[i];
            const v = vec3_1.Vec3.create(ya[bi][0], ya[bi][1], ya[bi][2]);
            vec3_1.Vec3.transformMat4(v, v, transform);
            const dx = v[0] - xa[ai][0];
            const dy = v[1] - xa[ai][1];
            const dz = v[2] - xa[ai][2];
            const distSq = dx * dx + dy * dy + dz * dz;
    ...
    refineWithCutoff(cutoff) {
        ...
        for (let iter = 0; iter < 10; iter++) {
            ...
            // Transform ya
            const yt = new Array(this.lenB);
            for (let i = 0; i < this.lenB; i++) {
                const v = vec3_1.Vec3.create(ya[i][0], ya[i][1], ya[i][2]);
                vec3_1.Vec3.transformMat4(v, v, transform);
                yt[i] = [v[0], v[1], v[2]];
            }
    ...
    extendFromSeed(initialTransform) {
        ...
        for (let iter = 0; iter < 20; iter++) {
            ...
            const yt = new Array(lenB);
            for (let i = 0; i < lenB; i++) {
                const v = vec3_1.Vec3.create(ya[i][0], ya[i][1], ya[i][2]);
                vec3_1.Vec3.transformMat4(v, v, currentTransform);
                yt[i] = [v[0], v[1], v[2]];
            }
    ...
    refineAlignment() {
        ...
        for (let iter = 0; iter < maxIterations; iter++) {
            ...
            const newPairedA = newAlignA.map(i => xa[i]);
            const newPairedB = newAlignB.map(j => ya[j]);
    ...
    nwdpScore(score, gapOpen) {
        ...
        if (this.bestAlignmentA.length >= 3) {
            ...
            this.bestScore = this.scoreTMWithCutoff(this.bestAlignmentA, this.bestAlignmentB, this.bestTransform, this.d0A, this.lenA, this.scoreD8);
        }
    }
    /**
    ...
    kabsch(a, b) {
        ...
        const result = minimize_rmsd_1.MinimizeRmsd.compute({ a: posA, b: posB });
