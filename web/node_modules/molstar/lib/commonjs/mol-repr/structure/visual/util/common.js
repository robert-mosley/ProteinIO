...
exports.checkSphereImpostorSupport = checkSphereImpostorSupport;
exports.checkCylinderImpostorSupport = checkCylinderImpostorSupport;
...
const functional_group_1 = require("../../../../mol-model-props/computed/chemistry/functional-group.js");
...
function checkSphereImpostorSupport(webgl) {
    if (!webgl) {
        if (debug_1.isDebugMode && !SphereImpostorWarningShown) {
        ...
        if (debug_1.isDebugMode && !SphereImpostorWarningShown) {
            const missing = [];
            if (!webgl.extensions.fragDepth)
                missing.push('fragDepth');
            if (!webgl.extensions.textureFloat)
...
function checkCylinderImpostorSupport(webgl) {
    if (!webgl) {
        if (debug_1.isDebugMode && !CylinderImpostorWarningShown) {
...
exports.CommonSurfaceParams = {
    ...
    includeParent: param_definition_1.ParamDefinition.Boolean(false, { description: 'Include elements of the parent structure in surface calculation to get a surface patch of the current structure. Note that this disables GPU support.' }),
};
