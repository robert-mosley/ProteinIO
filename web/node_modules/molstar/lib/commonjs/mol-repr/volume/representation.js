        ...
        if (volume_1.Volume.Cell.isLoci(loci)) {
            ...
            for (const { instances } of loci.elements) {
                ordered_set_1.OrderedSet.forEach(instances, j => {
                    if (apply(interval_1.Interval.ofSingleton(j)))
                        changed = true;
                });
            }
        }
            ...
            for (const { segments, instances } of loci.elements) {
                if (ordered_set_1.OrderedSet.has(segments, key)) {
                    ordered_set_1.OrderedSet.forEach(instances, j => {
                        if (apply(interval_1.Interval.ofSingleton(j)))
                            changed = true;
                    });
            ...
            if (interval_1.Interval.is(loci.instances)) {
                ...
                for (let i = 0, il = loci.instances.length; i < il; ++i) {
                    if (apply(interval_1.Interval.ofSingleton(i)))
                        changed = true;
                }
            }
        }
    ...
    function lociApply(loci, apply) {
        if ((0, loci_1.isEveryLoci)(loci)) {
            if (currentProps.instanceGranularity) {
                return apply(interval_1.Interval.ofBounds(0, locationIt.instanceCount));
            }
            else {
                return apply(interval_1.Interval.ofBounds(0, locationIt.groupCount * locationIt.instanceCount));
            }
        }
        else {
            if (currentProps.instanceGranularity) {
