...
function cameraProject(out, point, viewport, projectionView) {
    ...
    vec4_1.Vec4.set(tmpVec4, point[0], point[1], point[2], 1.0);
    // transform into clip space
    vec4_1.Vec4.transformMat4(tmpVec4, tmpVec4, projectionView);
    // transform into NDC
    const w = tmpVec4[3];
    if (w !== 0) {
        tmpVec4[0] /= w;
...
function cameraUnproject(out, point, viewport, inverseProjectionView) {
    ...
    out[1] = (2 * py) / height - 1;
    out[2] = 2 * pz - 1;
    return vec3_1.Vec3.transformMat4(out, out, inverseProjectionView);
}
