...
const column_1 = require("../mol-data/db/column.js");
...
function lociLabel(loci, options = {}) {
    ...
    switch (loci.kind) {
        ...
        case 'group-loci':
            ...
            return g ? loci.shape.getLabel(ordered_set_1.OrderedSet.start(g.ids), g.instance) : '';
        case 'every-loci':
            return 'Everything';
        case 'empty-loci':
            return 'Nothing';
        ...
        case 'cell-loci':
            const size = volume_1.Volume.Cell.getLociSize(loci);
            const start = ordered_set_1.OrderedSet.start(loci.elements[0].indices);
            ...
            if (size === 1) {
                if (loci.volume.instances.length > 1) {
                    label.push(`Instance #${ordered_set_1.OrderedSet.start(loci.elements[0].instances) + 1}`);
                }
                label.push(`${volume_1.Volume.IsoValue.toString(absVal)} (${volume_1.Volume.IsoValue.toString(relVal)})`);
            }
            return label.join(' | ');
        case 'segment-loci':
            const segmentCount = volume_1.Volume.Segment.getLociSize(loci);
            const segmentLabels = (_a = volume_1.Volume.Segmentation.get(loci.volume)) === null || _a === void 0 ? void 0 : _a.labels;
            const firstSegment = ordered_set_1.OrderedSet.start(loci.elements[0].segments);
...
function bondLabel(bond, options = {}) {
    return bundleLabel({ loci: [
            structure_1.StructureElement.Loci(bond.aStructure, [{ unit: bond.aUnit, indices: ordered_set_1.OrderedSet.ofSingleton(bond.aIndex) }]),
            structure_1.StructureElement.Loci(bond.bStructure, [{ unit: bond.bUnit, indices: ordered_set_1.OrderedSet.ofSingleton(bond.bIndex) }])
        ] }, options);
}
...
function _bundleLabel(bundle, options) {
    ...
    if (isSingleElements) {
        const locations = bundle.loci.map(l => {
            ...
            return structure_1.StructureElement.Location.create(l.structure, unit, unit.elements[ordered_set_1.OrderedSet.start(indices)]);
        });
...
function _atomicElementLabel(location, granularity, hideOccupancy = false) {
    ...
    const has_label_seq_id = location.unit.model.atomicHierarchy.residues.label_seq_id.valueKind(rI) === column_1.Column.ValueKinds.Present;
