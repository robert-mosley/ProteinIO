...
function getPolymerAsymIdSerialMap(structure) {
    ...
    for (let i = 0, il = structure.unitSymmetryGroups.length; i < il; ++i) {
        ...
        if (structure_1.Unit.isAtomic(unit)) {
            ...
            const chainIt = segmentation_1.Segmentation.transientSegments(chainAtomSegments, unit.elements);
            ...
            const chainIt = segmentation_1.Segmentation.transientSegments(chainElementSegments, unit.elements);
            while (chainIt.hasNext) {
                const { index: chainIndex } = chainIt.move();
