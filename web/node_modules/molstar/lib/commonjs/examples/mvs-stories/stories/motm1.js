...
const utils_1 = require("../../../extensions/mvs/helpers/utils.js");
const mvs_builder_1 = require("../../../extensions/mvs/tree/mvs/mvs-builder.js");
...
// 1pmb->1mbn
const align = mat4_1.Mat4.fromArray(mat4_1.Mat4.zero(), [0.4634187130865737, -0.7131589697034304, 0.5259728687171936, 0, -0.22944227902330105, -0.6698811108214233, -0.7061273127008398, 0, 0.8559202154942049, 0.2065522332899299, -0.4740643150728161, 0, -52.54880970106205, 37.49099778180445, -6.133850309914719, 1], 0);
// 1mbo->1myf
const alignmbo = mat4_1.Mat4.fromArray(mat4_1.Mat4.zero(), [-0.8334619943964441, -0.512838061396133, -0.20576353166796402, 0, -0.20145089001561267, 0.628743285359846, -0.7510655776229758, 0, 0.5145474196737698, -0.5845332204089626, -0.6273453801378679, 0, 11.864847328611186, -1.5261713438028912, 23.638919347623467, 1], 0);
const ill_color = (color, carbonLightness) => ({
    ...
    molstar_color_theme_params: {
        style: {
            name: 'uniform',
            params: {
                value: (0, utils_1.decodeColor)(color),
                saturation: 0,
                lightness: 0,
            }
        },
...
`;
const query1 = builder_1.MolScriptBuilder.struct.generator.atomGroups({
    'entity-test': builder_1.MolScriptBuilder.core.rel.eq([
        builder_1.MolScriptBuilder.struct.atomProperty.core.modelEntryId(),
        '1MBN'
    ])
});
const firstEntity1 = q((0, expression_formatter_1.formatMolScript)(query1), 'mol-script');
const query2 = builder_1.MolScriptBuilder.struct.generator.atomGroups({
    'entity-test': builder_1.MolScriptBuilder.core.rel.eq([
        builder_1.MolScriptBuilder.struct.atomProperty.core.modelEntryId(),
        '1PMB'
    ])
});
const firstEntity2 = q((0, expression_formatter_1.formatMolScript)(query2), 'mol-script');
const query3 = builder_1.MolScriptBuilder.struct.generator.atomGroups({
    'entity-test': builder_1.MolScriptBuilder.core.rel.eq([
        builder_1.MolScriptBuilder.struct.atomProperty.core.modelEntryId(),
        '1MBN'
    ]),
    'residue-test': builder_1.MolScriptBuilder.core.set.has([
        builder_1.MolScriptBuilder.set(12, 140, 87),
        builder_1.MolScriptBuilder.struct.atomProperty.macromolecular.auth_seq_id()
    ])
});
const charged_residues = q((0, expression_formatter_1.formatMolScript)(query3), 'mol-script');
...
const Steps = [
            ...
            _1mbo.component({ selector: { label_asym_id: 'C', auth_seq_id: 155 } })
                ...
                custom: {
                    ...
                    molstar_color_theme_params: {
                        carbonColor: {
                            name: 'uniform',
                            params: { value: (0, utils_1.decodeColor)(red2) }
                        },
                    }
                }
            });
    ...
    },
    {
        ...
        state: () => {
            ...
            const r44 = vec3_1.Vec3.create(22.300, 33.300, -6.200);
            const r47 = vec3_1.Vec3.create(23.200, 32.000, -8.400);
            getEllipse(builder, r44, r47, 'salt1');
            // 18 OE1 16.600 22.500 20.500
            // 77 NZ 14.100 23.600 22.200
            const r18 = vec3_1.Vec3.create(16.600, 22.500, 20.500);
            const r77 = vec3_1.Vec3.create(14.100, 23.600, 22.200);
...
function getEllipse(builder, pos1, pos2, ref) {
    const center = vec3_1.Vec3.add((0, vec3_1.Vec3)(), pos1, pos2);
    vec3_1.Vec3.scale(center, center, 0.5);
    const major_axis = vec3_1.Vec3.sub((0, vec3_1.Vec3)(), pos2, pos1);
    const z_axis = vec3_1.Vec3.create(0, 0, 1);
    // cross to get minor
    const minor_axis = vec3_1.Vec3.cross((0, vec3_1.Vec3)(), major_axis, z_axis);
