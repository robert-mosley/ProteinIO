...
exports.tmAlignStructures = tmAlignStructures;
exports.loadStructuresNoAlignment = loadStructuresNoAlignment;
exports.sequenceAlignStructures = sequenceAlignStructures;
 ...
 */
async function tmAlignStructures(plugin, pdbId1, chain1, pdbId2, chain2, color1 = 0x3498db, color2 = 0xe74c3c) {
    var _a, _b, _c, _d;
    await plugin.clear();
    ...
    const caQuery1 = (0, compiler_1.compile)(builder_1.MolScriptBuilder.struct.generator.atomGroups({
        'chain-test': builder_1.MolScriptBuilder.core.rel.eq([builder_1.MolScriptBuilder.struct.atomProperty.macromolecular.auth_asym_id(), chain1]),
        'atom-test': builder_1.MolScriptBuilder.core.rel.eq([builder_1.MolScriptBuilder.struct.atomProperty.macromolecular.label_atom_id(), 'CA'])
    }));
    const caQuery2 = (0, compiler_1.compile)(builder_1.MolScriptBuilder.struct.generator.atomGroups({
        'chain-test': builder_1.MolScriptBuilder.core.rel.eq([builder_1.MolScriptBuilder.struct.atomProperty.macromolecular.auth_asym_id(), chain2]),
        'atom-test': builder_1.MolScriptBuilder.core.rel.eq([builder_1.MolScriptBuilder.struct.atomProperty.macromolecular.label_atom_id(), 'CA'])
    }));
    ...
    const sel1 = structure_1.StructureSelection.toLociWithCurrentUnits(caQuery1(new structure_1.QueryContext(structure1Data)));
    const sel2 = structure_1.StructureSelection.toLociWithCurrentUnits(caQuery2(new structure_1.QueryContext(structure2Data)));
    const loci1 = structure_1.StructureElement.Loci.is(sel1) ? sel1 : structure_1.StructureElement.Loci.none(structure1Data);
    const loci2 = structure_1.StructureElement.Loci.is(sel2) ? sel2 : structure_1.StructureElement.Loci.none(structure2Data);
    if (structure_1.StructureElement.Loci.size(loci1) === 0 || structure_1.StructureElement.Loci.size(loci2) === 0) {
    ...
    // Run TM-align
    const result = (0, tm_align_1.tmAlign)(loci1, loci2);
 ...
 * @param color2 - Optional color for second structure (hex, default red)
 */
async function loadStructuresNoAlignment(plugin, pdbId1, chain1, pdbId2, chain2, color1 = 0x3498db, color2 = 0xe74c3c) {
    await plugin.clear();
    const url1 = `https://files.rcsb.org/download/${pdbId1}.pdb`;
    const url2 = `https://files.rcsb.org/download/${pdbId2}.pdb`;
    const label1 = `${pdbId1} Chain ${chain1}`;
 ...
 * @param color2 - Optional color for second structure (hex, default red)
 */
async function sequenceAlignStructures(plugin, pdbId1, chain1, pdbId2, chain2, color1 = 0x3498db, color2 = 0xe74c3c) {
    var _a, _b, _c, _d;
    await plugin.clear();
    const url1 = `https://files.rcsb.org/download/${pdbId1}.pdb`;
    const url2 = `https://files.rcsb.org/download/${pdbId2}.pdb`;
    ...
    const transforms = (0, superposition_1.alignAndSuperpose)([sel1, sel2]);
    // Apply the transformation to superimpose structure 2 onto structure 1
