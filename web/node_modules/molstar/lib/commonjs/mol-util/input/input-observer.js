...
const bit_flags_1 = require("../bit-flags.js");
...
(function (ButtonsType) {
    ButtonsType.has = bit_flags_1.BitFlags.has;
    ButtonsType.create = bit_flags_1.BitFlags.create;
    let Flag;
    (function (Flag) {
        ...
        /** Trigger button */
        Flag[Flag["Trigger"] = 32] = "Trigger";
        /** Squeeze button */
        Flag[Flag["Squeeze"] = 48] = "Squeeze";
...
function createEvents() {
    return {
        ...
        trackedPointers: new rxjs_1.Subject(),
    };
}
...
(function (InputObserver) {
    function create(props = {}) {
        ...
        return {
            ...
            pixelRatio: 1,
            ...createEvents(),
            setPixelScale: index_1.noop,
            updateTrackedPointers: index_1.noop,
            updateScreenTouches: index_1.noop,
            requestPointerLock: index_1.noop,
            exitPointerLock: index_1.noop,
            dispose: index_1.noop
        };
    }
    ...
    function fromElement(element, props = {}) {
        ...
        const pointerDown = (0, vec2_1.Vec2)();
        const pointerStart = (0, vec2_1.Vec2)();
        const pointerEnd = (0, vec2_1.Vec2)();
        const pointerDelta = (0, vec2_1.Vec2)();
        const rectSize = (0, vec2_1.Vec2)();
        const modifierKeys = {
            shift: false,
            alt: false,
            control: false,
        ...
        const trackedPointerState = {
            primary: { prev: undefined, down: (0, vec2_1.Vec2)(), end: (0, vec2_1.Vec2)(), axes: [] },
            secondary: { prev: undefined, down: (0, vec2_1.Vec2)(), end: (0, vec2_1.Vec2)(), axes: [] },
            distance: -1,
        };
        function buttonUp(flag, buttons, prevButtons) {
        ...
        function handleTrackedPointer(trackedPointer, isSecondary) {
            ...
            if (prevPressed && !pressed) {
                vec2_1.Vec2.set(state.end, x, y);
                if (vec2_1.Vec2.distance(state.end, state.down) < 20) {
            ...
            if (pressed) {
                ...
                if (isStart) {
                    vec2_1.Vec2.set(state.down, x, y);
                }
            }
            const keyValue = { x: 0, y: 0, pageX: 0, pageY: 0, modifiers, preventDefault: () => { } };
            if (buttonUp(ButtonsType.Flag.Forth, buttons, prev === null || prev === void 0 ? void 0 : prev.buttons)) {
                const forth = isSecondary ? 'GamepadX' : 'GamepadA';
                keyUp.next({ ...keyValue, code: forth, key: forth });
            }
            if (buttonUp(ButtonsType.Flag.Five, buttons, prev === null || prev === void 0 ? void 0 : prev.buttons)) {
        ...
        function handleTrackedPointerGesture(primary, left, button) {
            const d = vec3_1.Vec3.distance(primary.ray.origin, left.ray.origin);
        ...
        function handleTrackedPointers(trackedPointers) {
            let primary;
            let secondary;
            ...
            if ((primary === null || primary === void 0 ? void 0 : primary.buttons) === ButtonsType.Flag.Primary && (secondary === null || secondary === void 0 ? void 0 : secondary.buttons) === ButtonsType.Flag.Primary) {
                handleTrackedPointerGesture(primary, secondary, ButtonsType.Flag.Trigger);
            }
            else if ((primary === null || primary === void 0 ? void 0 : primary.buttons) === ButtonsType.Flag.Secondary && (secondary === null || secondary === void 0 ? void 0 : secondary.buttons) === ButtonsType.Flag.Secondary) {
                handleTrackedPointerGesture(primary, secondary, ButtonsType.Flag.Squeeze);
            }
            else {
                if (trackedPointerState.distance === -1) {
                    if (primary)
                        handleTrackedPointer(primary, false);
                    if (secondary)
                        handleTrackedPointer(secondary, true);
                }
                // end gesture only if all button are released
                if (!(primary === null || primary === void 0 ? void 0 : primary.buttons) && !(secondary === null || secondary === void 0 ? void 0 : secondary.buttons)) {
                    trackedPointerState.distance = -1;
                }
            }
        }
        let screenTouchPrev = undefined;
        let screenTouchStart = undefined;
        let screenTouchDistance = -1;
        function handleScreenTouches(screenTouches) {
            if (screenTouches.length === 2) {
                const a = vec2_1.Vec2.fromObj(screenTouches[0]);
                const b = vec2_1.Vec2.fromObj(screenTouches[1]);
                const d = vec2_1.Vec2.distance(a, b);
            ...
            if (!t) {
                if (screenTouchStart && screenTouchPrev) {
                    const a = vec2_1.Vec2.fromObj(screenTouchStart);
                    const b = vec2_1.Vec2.fromObj(screenTouchPrev);
                    if (vec2_1.Vec2.distance(a, b) < 10) {
                        const [x, y] = a;
                        const [pageX, pageY] = [x, y];
                        const modifiers = ModifiersKeys.create();
                        const button = ButtonsType.Flag.Trigger;
        ...
        const singleTouchPosition = (0, vec2_1.Vec2)(), singleTouchTmp = (0, vec2_1.Vec2)();
        function updateSingleTouchDistance(ev) {
            if (singleTouchDistance < 0)
                return;
            vec2_1.Vec2.set(singleTouchTmp, ev.touches[0].pageX, ev.touches[0].pageY);
            singleTouchDistance += vec2_1.Vec2.distance(singleTouchPosition, singleTouchTmp);
            vec2_1.Vec2.copy(singleTouchPosition, singleTouchTmp);
        }
        const firstTouchStart = (0, vec2_1.Vec2)();
        ...
        function onTouchStart(ev) {
            ...
            if (ev.touches.length === 1) {
                ...
                vec2_1.Vec2.set(singleTouchPosition, ev.touches[0].pageX, ev.touches[0].pageY);
                lastSingleTouch = ev.touches[0];
                onPointerDown(ev.touches[0]);
                vec2_1.Vec2.copy(firstTouchStart, pointerStart);
        ...
        function onPointerDown(ev) {
            ...
            vec2_1.Vec2.copy(pointerDown, pointerStart);
        ...
        function onPointerUp(ev) {
            ...
            if (!hasMoved && vec2_1.Vec2.distance(pointerEnd, pointerDown) < 4) {
        ...
        function onPointerMove(ev) {
            ...
            vec2_1.Vec2.div(pointerDelta, vec2_1.Vec2.sub(pointerDelta, pointerEnd, pointerStart), getClientSize(rectSize));
            if (vec2_1.Vec2.magnitude(pointerDelta) < common_1.EPSILON)
                return;
            const isStart = dragging === DraggingState.Started;
            if (isStart && !mask(ev.clientX, ev.clientY))
                return;
            if (vec2_1.Vec2.distance(pointerEnd, pointerDown) >= 4) {
                hasMoved = true;
            }
            const [dx, dy] = pointerDelta;
            drag.next({ x, y, dx, dy, pageX, pageY, buttons, button, modifiers: getModifierKeys(), isStart });
            vec2_1.Vec2.copy(pointerStart, pointerEnd);
