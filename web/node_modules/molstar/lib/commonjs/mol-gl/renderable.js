...
const s3fromArray = sphere3d_1.Sphere3D.fromArray;
            ...
            if (values.lodLevels.ref.version !== lodLevelsVersion) {
                for (let i = 0, il = lodLevels.length; i < il; ++i) {
                    if (mdbDataList[i].uniforms.length !== 1) {
                        ...
                        mdbDataList[i].uniforms[0] = ['uLod', value_cell_1.ValueCell.create((0, vec4_1.Vec4)())];
                    }
                    value_cell_1.ValueCell.update(mdbDataList[i].uniforms[0][1], vec4_1.Vec4.set(mdbDataList[i].uniforms[0][1].ref.value, lodLevels[i][0], lodLevels[i][1], lodLevels[i][2], lodLevels[i][4]));
                }
    ...
    return {
        ...
        render: (variant, sharedTexturesCount) => {
            if (values.uAlpha && values.alpha) {
                value_cell_1.ValueCell.updateIfChanged(values.uAlpha, (0, interpolate_1.clamp)(values.alpha.ref.value * state.alphaFactor, 0, 1));
            }
