...
const interval_1 = require("./interval.js");
const ordered_set_1 = require("./ordered-set.js");
const sorted_array_1 = require("./sorted-array.js");
var SortedRanges;
(function (SortedRanges) {
    function ofSortedRanges(array) { return sorted_array_1.SortedArray.ofSortedArray(array); }
    ...
    function firstIntersectionIndexFrom(ranges, set, from) {
        if (minAt(ranges, from) > ordered_set_1.OrderedSet.max(set) || max(ranges) < ordered_set_1.OrderedSet.min(set))
            return -1;
        for (let i = from, il = count(ranges); i < il; ++i) {
            const interval = interval_1.Interval.ofRange(minAt(ranges, i), maxAt(ranges, i));
            if (ordered_set_1.OrderedSet.areIntersecting(interval, set))
                return i;
        }
        return -1;
    }
    ...
    class Iterator {
        updateValue() {
            ...
            this.value.start = ordered_set_1.OrderedSet.findPredecessorIndex(this.set, startAt(this.ranges, this.curIndex));
            this.value.end = ordered_set_1.OrderedSet.findPredecessorIndex(this.set, endAt(this.ranges, this.curIndex));
        }
        move() {
            if (this.hasNext) {
                this.updateValue();
...
})(SortedRanges || (exports.SortedRanges = SortedRanges = {}));
