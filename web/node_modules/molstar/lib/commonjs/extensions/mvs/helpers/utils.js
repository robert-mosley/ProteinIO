...
exports.GroupedArray = exports.NumberMap = exports.MultiMap = void 0;
...
exports.getMVSReferenceObject = getMVSReferenceObject;
...
function decodeColor(colorString) {
    if (typeof colorString === 'number') {
        return (0, color_1.Color)(colorString);
    }
    return (0, utils_1.decodeColor)(colorString);
}
...
function getMVSReferenceObject(type, dependencies, ref) {
    if (!dependencies)
        return undefined;
    for (const key of Object.keys(dependencies)) {
        const o = dependencies[key];
...
exports.GroupedArray = {
    ...
    groupIndices(elements, group_by) {
        ...
        const elementIndices = (0, array_1.range)(elements.length).sort((i, j) => groups[i] - groups[j]);
        const offsets = [];
        for (let i = 0; i < elements.length; i++) {
