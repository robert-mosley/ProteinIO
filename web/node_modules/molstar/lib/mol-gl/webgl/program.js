 ...
 */
import { addShaderDefines } from '../shader-code.js';
import { getUniformSetters, getUniformType, isArrayUniform } from './uniform.js';
import { getAttribType } from './buffer.js';
    ...
    for (let i = 0; i < attribCount; ++i) {
        ...
        if (info) {
            ...
            if (name === 'gl_ViewID_OVR')
                continue; // OVR_multiview2 built-in
...
function normalizeVariant(variant) {
    if (typeof variant !== 'string')
        throw new Error(`unknown program variant: ${variant}`);
    if (variant.startsWith('color'))
        return 'color';
    if (variant.startsWith('pick'))
        return 'pick';
    return variant;
}
export function createProgram(gl, state, extensions, parameters, getShader, props) {
    ...
    return {
        ...
        use: () => {
            ...
            if (isDebugMode && !finalized)
                throw new Error(`program not finalized: ${variant}`);
