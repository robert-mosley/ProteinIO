  ...
  return (
    <aside className="w-56 flex-shrink-0 border-r border-[#0f2040] hidden md:flex flex-col overflow-hidden">
      <div className="flex-1 overflow-y-auto p-3 space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2 px-1 mb-2">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-600">Recent</span>
          </div>
          {history.length === 0 ? (
            <div className="px-2 py-3 rounded-lg border border-dashed border-[#1a3355] text-xs text-slate-700 text-center">
              No searches yet
            </div>
          ) : (
            <div className="space-y-0.5">
              {history.map((h) => (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-2.5 py-2 rounded-lg text-left hover:bg-[#0d1b30] text-slate-400 hover:text-cyan-400 transition-all duration-100 group"
                >
                  <ChevronRight className="w-3 h-3 text-slate-700 group-hover:text-cyan-500 flex-shrink-0" />
        ...
        <div>
          <div className="flex items-center gap-2 px-1 mb-2">
            ...
            <span className="text-[10px] font-semibold uppercase tracking-widest text-slate-600">Data Sources</span>
          </div>
          <div className="space-y-0.5">
            {[
              ...
              <div key={src.name} className="flex items-center gap-2.5 px-2.5 py-2 rounded-lg">
                ...
                <span className="text-xs text-slate-500">{src.name}</span>
      ...
      <div className="p-3 border-t border-[#0f2040]">
        <div className="text-[10px] text-slate-700 text-center font-mono">ProteinIO v0.1</div>
      </div>
    </aside>
  )
}
